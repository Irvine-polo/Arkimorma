<?php 
// include('includes/AdminMonthlyPayment.php');
 ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>Arkimorma | Homepage</title>

    <link rel="stylesheet" type="text/css" href="assets/plugins/bootstrap/css/bootstrap.css">
    <link rel="stylesheet" type="text/css" href="assets/plugins/fontawesome/css/all.css">
    <link href="https://fonts.googleapis.com/css2?family=Anton&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="assets/css/style.css">
    <link rel="icon" href="data:image/svg+xml;base64,">
</head>
<body>
        <?php include('includes/navigation.php'); ?>
    <!-- Banner -->
    <section id="banner">
            <div class="index-intro">
                
                
            </div>

            <div class="jumbotron text-center">
                <div class="row">
                <div class="col-sm-5 text-right">
                <img src="assets/images/arkimorma Logo.jpg" class="img-circle " style="width: 20%;">
                </div>

                <div class="col-sm-7 text-left" style="padding-top: 5px;" >
                <h1>ARKlMORMA</h1>
                <p>An online rental website where you can browse shop and rent!</p>
                </div>
               
            </div>
            </div>
           
    </section>
    <!-- Banner -->


    <!-- Services -->
        <div class="container">
            <div class="OurServices text-center">
                <h3>Our Services</h3>
                
            </div>
            
        </div>
        <section>
            <div class="container">
                <div class="row">
                    <div class="method">
                    <div class="col-md-4 text-center"> 
                        <div class="">
                                    <i class="fab fa-shopify fa-4x" style="color: #8d5e45"></i>
                                 <div class="caption">
                                    <h3>Shop</h3>
                                    <p>Browse item with different designs in different shops</p>      
                                </div> 
                        </div>
                    </div>
                    
                    
                    <div class="col-md-4 text-center"> 
                         <div class="">
                              <i class="fas fa-cart-plus fa-4x" style="color: #8d5e45"></i>
                                 <div class="caption">
                                    <h3>Rent</h3>
                                    <p>Add items to your cart with the item of your choice</p>      
                                </div> 
                        </div>
                    </div>

                    <div class="col-md-4 text-center"> 
                        <div class="">
                                <<i class="fas fa-money-bill-wave-alt fa-4x" style="color: #8d5e45"></i>
                                    <div class="caption">
                                        <h3>Payment Method</h3>
                                        <p>Cash on delivery or online payment method</p>      
                                    </div> 
                        </div>

                    </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- Services -->

        <!-- Choices -->
        <section>
            <div class="container-fluid">
                <div class="row text-center">
                    <div class="choices">
                        <div class="col-md-4 text-center">
                                    <div class="wear">
                                        <img src="assets/images/index2.jpg" class="img-responsive img-rounded img-thumbnail">
                                        <h5>Fashion</h5>
                                    </div>
                            </div>
                        
                
                            <div class="col-md-4 text-center">
                                    <div class="wear">
                                        <img src="assets/images/index3.jpg" class="img-responsive img-rounded img-thumbnail">
                                        <h5>Occasionwear</h5>
                                    </div>
                            </div>

                            <div class="col-md-4 text-center " >
                                    <div class="wear">
                                        <img src="assets/images/index1.jpg " class="img-responsive img-rounded img-thumbnail">
                                        <h5>Corporate</h5>
                                    </div>
                            </div>
                        
                    </div>
                
                 </div>
                 </div>
        </section>

        <!-- Choices -->

        <!-- TextSeperator -->

        <div class="container">
            <div class="Believe text-center ">
                <h3 style="color: #000;">We think there's a better way to get dressed.</h3>
                <p>We're committed to making fashion more accessible by offering a sustainable approach to occasionwear. Hello, and welcome to our closet!</p>
            </div>
        </div>

        <!-- TextSeperator -->

        <!-- products -->
        <div class="container">
         <div class="row">
                    <div class=" col-md-3">
                            <div class="thumbnail">
                                <div class="prod">
                                    <img src="assets/images/gowns1.jpg">
                                        <div class="caption">
                                            <h3>Green pearl</h3>
                                            <p>A green gown with shinning pearl and makes you look attractive</p>
                                        
                                        </div>
                                </div>
                            </div>
                        </div>

                    <div class=" col-md-3">
                        <div class="thumbnail">
                            <div class="prod">
                                <img src="assets/images/gowns2.jpg">
                                    <div class="caption">
                                        <h3>Yellow dash</h3>
                                        <p>A very attractive gown that symbolized strenght</p>
                                    
                                    </div>
                            </div>
                        </div>
                    </div>

                    <div class=" col-md-3">
                        <div class="thumbnail">
                            <div class="prod">
                                <img src="assets/images/gowns5.jpg">
                                    <div class="caption">
                                        <h3>Cotton candy</h3>
                                        <p>A very lovely gown for teens in any ocassion.</p>
                                    
                                    </div>
                            </div>
                        </div>
                    </div>

                    <div class=" col-md-3">
                        <div class="thumbnail">
                                <div class="prod">
                                    <img src="assets/images/gowns4.jpg">
                                        <div class="caption">
                                            <h3>Dusk gown</h3>
                                            <p>A gown that will make you look fierce in every aspect.</p>
                                        </div>
                                    
                                </div>
                        </div>
                    </div>


            
            </div>

            <div class="row">
                    <div class=" col-md-3">
                            <div class="thumbnail">
                                <div class="prod">
                                    <img src="assets/images/formal1.jpg">
                                        <div class="caption">
                                            <h3>Grey coat</h3>
                                            <p>A coat for evey passionate man, comfortable to wear.</p>
                                        
                                        </div>
                                </div>
                            </div>
                        </div>

                    <div class=" col-md-3">
                        <div class="thumbnail">
                            <div class="prod">
                                <img src="assets/images/formal2.jpg">
                                    <div class="caption">
                                        <h3>Dark coat</h3>
                                        <p>Good for young ones. Can wear in any events such as weddings.</p>
                                    
                                    </div>
                            </div>
                        </div>
                    </div>

                    <div class=" col-md-3">
                        <div class="thumbnail">
                            <div class="prod">
                                <img src="assets/images/formal3.jpg">
                                    <div class="caption">
                                        <h3>Vintage coat</h3>
                                        <p>A coat that you can wear in a vintage or dreamy theme.</p>
                                    
                                    </div>
                            </div>
                        </div>
                    </div>

                    <div class=" col-md-3">
                        <div class="thumbnail">
                                <div class="prod">
                                    <img src="assets/images/formal4.jpg">
                                        <div class="caption">
                                            <h3>Tacky coat</h3>
                                            <p>Trending in young ones. A cool looking coat that makes you handsome.</p>
                                        </div>
                                    
                                </div>
                        </div>
                    </div>


            
            </div>
        </div>
        <!-- products -->
        
        <!-- two ways to rent -->
        
        <div class="container">
                <div class="waytorent">
                    <h3> How to rent?</h3>
                    <HR>
                </div>
                
        </div>
        
        
        <div class="container">
            
           
            <div class="row">
                    <div class="col-md-3 text-center">
                    <div class="thumbnail">
                    <div class="browse">
                    <i class="fas fa-user-circle fa-3x" style="color: #8d5e45"></i>
                        <h5 class="text-center">Login</h5>
                        <p>Register an account. After you register go to login</p>
                
                        <!-- <button type="button" class="btn btn-link ">Rent now</button> -->
                    </div>
                </div>
                </div>

                <div class="col-md-3 text-center">
                <div class="thumbnail">
                    <div class="browse">
                    <i class="fas fa-store fa-3x" style="color: #8d5e45"></i>
                        <h5>Shop</h5>
                        <p>After you login click the shop button and choose the shop that you like</p>
                    </div>
                </div>
            </div>

                <div class="col-md-3 text-center">
                <div class="thumbnail">
                    <div class="browse">
                    <i class="fas fa-shopping-cart fa-3x" style="color: #8d5e45"></i>
                        <h4>Add item to cart</h4>
                        <p>After clicking the shop choose the item of your choice and add it to your cart</p>
                    </div>
                </div>
                </div>

                <div class="col-md-3 text-center">
                <div class="thumbnail">
                    <div class="browse">
                    <i class="fas fa-truck-moving fa-3x" style="color: #8d5e45" ></i>
                        <h4>Rent</h4>
                        <p>Wait for your item to be process and to be ready to rent</p>
                    </div>
                </div>
            </div>
</div>
                <br>
                
            <div class="easy text-center">
            
                

            </div>
            
        </div>
        <div class="ending">
        <div class="div">
        <img src="assets/images/wave2.png" class="top-img">
        </div>
        <!-- two ways to rent -->
        <section id="banner">
            <div class="index-intro1">
                <div class="container">
                    <div class="row">
                        <div class="col-md-6">
                            <p class="text-title">Arkimorma</p>
                            <p>Arkilaang Pamorma: An online rental website where you can browse, shop and rent.</p>
                           <a href="UserLogin.php"> <button type="button" class="btn btn-warning">Shop now</button></a>
                        </div>

                        <div class="col-md-6">
                            <img src="assets/images/q2.png" class="bannerImg">

                        </div>
                    </div>
                </div>
            </div>

           
        </section>
                        

                        
                        
                    </div>
                </div>
            </div>
            
            
        </section>
        </div>
        <?php include('includes/footer.php'); ?>



         

           

    <script src="assets/js/jquery.js"></script>
    <script src="assets/plugins/bootstrap/js/bootstrap.js"></script>
    <script src="assets/plugins/fontawesome/js/fontawesome.js"></script>

</body>
</html>