
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<title>Arkimorma | About</title>

    <link rel="stylesheet" type="text/css" href="assets/plugins/bootstrap/css/bootstrap.css">
    <link rel="stylesheet" type="text/css" href="assets/plugins/fontawesome/css/all.css">
    <link href="https://fonts.googleapis.com/css2?family=Anton&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="assets/css/style.css">
     <link rel="icon" href="data:image/svg+xml;base64,">
</head>

<body>
<?php include('includes/navigation.php'); ?>
        
        <div class="about">
			<div class="row">
				<div class="background1">   
					<div class="col-xs-6">
						<div class="image">
							<img src="assets/images/about1.jpg" alt="" >
							</div>
					</div>
							<div class="col-xs-6 " style="padding-top: 10%">
								<h4>About Us Arkimorma: Arkilaang Pamorma</h4>
								<p>"Arkimorma is a online website for online rental gowns and formal suits"
									It tends to give people an easy access for the clothes that they want to wear on a special events.Arkimorma consist of different 
									clothes categories such as dress, wedding gowns, suits and many more.Arkimorma website is created by these people.</p>
							
							</div>
						
				</div>
			</div>
			
		
			<div class="row">
				<div class="col-sm-6 col-md-4" style="padding-top: 3%">
						<div class="thumbnail" >
								<h4><b>Diana Vreeland</b></h4>
									<p>"Fashion is part of the daily air and it changes all the time, with all the events. You can even see the approaching of a revolution in clothes. 
										You can see and feel everything in clothes."  </p>
									
					
						</div>
				</div>

				<div class="col-sm-6 col-md-4" style="padding-top: 3%">
						<div class="thumbnail" >
								<h4><b>Alexander McQueen</b></h4>
									<p>"I think there is beauty in everything. What 'normal' people perceive as ugly, I can usually see something of beauty in it  </p>
									
						</div>
				</div>

				<div class="col-sm-6 col-md-4" style="padding-top: 3%">
						<div class="thumbnail" >
								<h4><b>Alexander Wang</b></h4>
									<p>"Anyone can get dressed up and glamorous, but it is how people dress in their days off that are the most intriguing."  </p>
									
					
						</div>
				</div>
			</div>
					
						
				
			
			
			

			<div class="container-fluid padding">
					<div class="row" style="padding-top: 3%">

					<div class="col-xs-6 text-center">
								<img src="assets/images/team3.jpg" class="img-fluid" style=width:80%; >
							</div>

							<div class="col-xs-6 ">
								<h4><b>Why Rental?<b></h4>
								<p>If you are searching for getting a prom dress design you won't ever wear again for every other occasion,
									a gown rental shop may emerge as perfect option for you. A costume that matches your style. </p>
								<p>Dress well is a form of good manner. Life is too short to wear boring dresses. </br>
								If you hire a costume on rent then it has lots of benefits such as it saves time and money, you can try a new brand and new style.<br>
								So, take costumes on rent is a very good idea.  Your fashion is our goal. Costumes that helps you to make charming, Costume for every trend .
                                </p>
								<p>Beautiful costumes for beautiful ladies. Make your occasion memorable, Keep stylish and stay attractive, Cleanliness is our priority <br>
								We make you confident, Dress in your way. Clothes that change your day </p>
								<br>
								<a href="UserArkimormaShop.php" class="btn btn-primary">Learn more</a>
							</div>

							
							
							
						</div>
					</div>
					<hr class="my-4">
			

			 <div class="our-team" >
				<div class="container">
					<h3 class="text-center" >OUR TEAM</h3>
					<h4> Students of City College Of Angeles</h4>
					<div class="row">

						<div class="col-xs-3">
							<div class="image img img-thumbnail">
								<img src="assets/images/Eugene.jpg" alt="">
							</div>
							<div class="description">
								<div class="name">
									<h3>Quiambao,Eugene</h3>
								</div>
								<div>
									<h5> Bachelor of Science In Information System</h5>
								</div>
								<div class="social-icons">
									<a href="#"><i class="fab fa-facebook text-primary"></i></a>
									<a href="#"><i class="fab fa-instagram text-danger"></i></a>
									<a href="#"><i class="fab fa-twitter text-info"></i></a>
								</div>
							</div>
						</div>

						<div class="col-xs-3">
							<div class="image img img-thumbnail">
								<img src="assets/images/Irvine.jpg" alt="">
							</div>
							<div class="description">
								<div class="name">
									<h3>Zurbano, Irvine</h3>
								</div>
								<div>
								<h5> Bachelor of Science In Information System</h5>
								</div>
								<div class="social-icons">
									<a href="#"><i class="fab fa-facebook text-primary"></i></a>
									<a href="#"><i class="fab fa-instagram text-danger"></i></a>
									<a href="#"><i class="fab fa-twitter text-info"></i></a>
								</div>
							</div>
						</div>

						<div class="col-xs-3">
							<div class="image img img-thumbnail">
								<img src="assets/images/Amy.jpg" alt="">
							</div>
							<div class="description">
								<div class="name">
									<h3>Sta. Ana Amy</h3>
								</div>
								<div>
								<h5> Bachelor of Science In Information System</h5>
								</div>
								<div class="social-icons">
									<a href="#"><i class="fab fa-facebook text-primary"></i></a>
									<a href="#"><i class="fab fa-instagram text-danger"></i></a>
									<a href="#"><i class="fab fa-twitter text-info"></i></a>
								</div>
							</div>
						</div>

						<div class="col-xs-3">
							<div class="image img img-thumbnail">
								<img src="assets/images/Mariela.jpg" alt="">
							</div>
							<div class="description">
								<div class="name">
									<h3>Razon,Mariela</h3>
								</div>
								<div>
								<h5> Bachelor of Science In Information System</h5>
								</div>
								<div class="social-icons">
									<a href="#"><i class="fab fa-facebook text-primary"></i></a>
									<a href="#"><i class="fab fa-instagram text-danger"></i></a>
									<a href="#"><i class="fab fa-twitter text-info"></i></a>
								</div>
							</div>
						</div>


					</div>
				</div>
			</div>
		


		</div>
        <!-- ABOUT -->
        






        <?php include('includes/footer.php'); ?>

        <<script src="assets/js/jquery.js"></script>
<script src="assets/plugins/bootstrap/js/bootstrap.js"></script>
<script src="assets/plugins/fontawesome/js/fontawesome.js"></script>
</body>
</html>