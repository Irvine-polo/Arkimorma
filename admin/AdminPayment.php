...
<head>
	...
	<title>Arkimorma | Admin-Commssion Payment</title>
...
<?php include('../includes/AdminMonthlyPayment.php'); ?>
...
$sql = $dbconnnect->query("SELECT * FROM payment WHERE MONTH(order_date_completed) = '$today'");
...
while ($row = mysqli_fetch_assoc($sql)) {
  ...
  <tr>
    ...
    <td><?php $date_completed = $row['order_date_completed']; $timestamp = strtotime($date_completed);$new_date = date('F d Y H:m:sa' , $timestamp); echo$new_date  ?></td>

    
    <td><?php $price = $row['order_price']; $price = asDollars($price); echo $price;?></td>
    <td><?php echo$row['shop_id']; ?></td>
    <td><?php $fee = $row['fee'];  $fee = asDollars($fee); echo $fee;?></td>
  </tr>
...
echo"</table>";
$income = asDollars($income);
echo"<h5> Total : ".$income."</h5>";
echo"<h5> Total order made : ".$count."</h5>";
 ?>
...
<?php echo$message; ?>
    <table  id="income">
      ...
      <tbody>
            ...
            <form method="POST" action="AdminPayment.php">
              <select name="months">
                <option selected>Choose Month</option>
            <?php 
            $sql = $dbconnnect->query("SELECT * FROM monthly_payment");
            while ($row = mysqli_fetch_assoc($sql)) {
             ?>
        ...
        if (isset($_POST['month_sort'])) {
          ...
          $com = $dbconnnect->query("SELECT * FROM monthly_payment WHERE month = '$month'");
          
        }
        else{
           $com = $dbconnnect->query("SELECT * FROM monthly_payment");
        }
          ...
          while ($row = mysqli_fetch_assoc($com)) {
            ?>
              <tr>
                ...
                <td>Income Statement for <?php echo$row['m']." Year ".$row['year']; ?> </td>
                <td><?php $income = $row['net_income']; $income = asDollars($income); echo$income;?></td>
                <td><a href="AdminStatementIncome.php?Statement=<?php echo$row['no']; ?>&Month=<?php echo$row['m']; ?>&year=<?php echo$row['year']; ?>&income=<?php echo$row['net_income']; ?>">View</a></td>
