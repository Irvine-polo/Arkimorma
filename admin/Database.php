...
<head>
	...
	<title>Arkimorma | Admin-Database</title>
...
<body>
	...
	<div class="container">
			...
			<?php 
			$sql = $dbconnnect->query("SELECT * FROM dbbackup ORDER BY `date_backup` DESC ");
			while ($row = mysqli_fetch_assoc($sql)) {
				$id = $row['id'];
				$filename = $row['filename'];
				$date = $row['date_backup'];
				?>
				<tr>
					<td><?php echo$id ?></td>
					<td><?php echo$filename ?></td>
					<td><?php $date_backup = $date ; $timestamp = strtotime($date_backup);$new_date = date('F d Y H:m:sa', $timestamp); echo$new_date ?></td>
