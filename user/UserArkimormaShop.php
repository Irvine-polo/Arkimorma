<?php    
include('../includes/user-navigation.php'); 
require_once'dbcon.php';



function asDollars($value) {
		 if ($value<0) return "-".asDollars(-$value);
		 return '₱ ' . number_format($value);
	 }

 if (isset($_GET['shop'])) {
	 $shop = $shoptarget;
 }

//days remaining process

 ?>

<!DOCTYPE html>
<html>
<head>
	 <meta charset="utf-8">
	 <meta name="viewport" content="width=device-width, initial-scale=1">

	 <title>Arkimorma | Shop</title>
	 <link rel="stylesheet" type="text/css" href="../assets/plugins/bootstrap/css/bootstrap.css">
	 <link rel="stylesheet" type="text/css" href="../assets/plugins/fontawesome/css/all.css">
	 <link rel="preconnect" href="https://fonts.googleapis.com">
	 <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
	 <link href="https://fonts.googleapis.com/css2?family=Anton&display=swap" rel="stylesheet">
	 <link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap" rel="stylesheet">
	 <link rel="stylesheet" type="text/css" href="../assets/css/style.css">
	 <link rel="stylesheet" type="text/css" href="../assets/css/seller-style.css">

	 <link rel="preconnect" href="https://fonts.googleapis.com">
	<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
	<link href="https://fonts.googleapis.com/css2?family=Lobster&family=Open+Sans:wght@300;400&display=swap" rel="stylesheet">

	<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css">
	<link href="https://fonts.googleapis.com/css?family=Merriweather:400,900,900i" rel="stylesheet">
	<script src="https://code.jquery.com/jquery-3.6.0.min.js" integrity="sha256-/xUj+3OJU5yExlq6GSYGSHk7tPXikynS7ogEvDej/m4=" crossorigin="anonymous"></script>


</head>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.3/jquery.min.js"></script>
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
 <link rel="icon" href="data:image/svg+xml;base64,">
<style type="text/css">
	#button {
	  display: inline-block;
	  background-color: #1D6B97;
	  width: 50px;
	  height: 50px;
	  text-align: center;
	  border-radius: 4px;
	  position: fixed;
	  bottom: 30px;
	  right: 30px;
	  transition: background-color .3s, 
	    opacity .5s, visibility .5s;
	  opacity: 0;
	  visibility: hidden;
	  z-index: 1000;
	  border-radius: 50px;
	}
	#button::after {
	  content: "\f077";
	  font-family: FontAwesome;
	  font-weight: normal;
	  font-style: normal;
	  font-size: 2em;
	  line-height: 50px;
	  color: #fff;
	}
	#button:hover {
	  cursor: pointer;
	  background-color: #333;
	}
	#button:active {
	  background-color: #555;
	}
	#button.show {
	  opacity: 1;
	  visibility: visible;
	}

	


	 /*li{
			list-style-type:none;
			margin-top: -10px;
	 }*/


	 #proimg{
			 overflow: hidden;
	 }

	 
	 #proimg img{
			max-height: 400px;
			width: 100%;
			object-fit: cover;
	 }

/*	 .container{
			display: flex;
			
	 }*/


	  .shop_content{
	 	transition: transform .2s;


	 }

	 .shop_content:hover{
	 	background: #EAECEE;
	 	 -ms-transform: scale(1.5); /* IE 9 */
		  -webkit-transform: scale(0.5); /* Safari 3-8 */
		  transform: scale(1.1); 
		


	 }

	 .content b ,p{
			margin-top: 3px;
			margin-left: 5px;
			font-size: 16px;

	 }

	 .content a{
			text-decoration: none;
			color: black;
	 }

	 .content p{
			color: grey;
	 }


	 .checked {
		 color: orange;
	 }
img{
	 height: 100px;
	 width: 100px;
}


</style>
<body>
	<!--- back to top -->
	<a id="button"></a>

<?php 
$user_id = $_SESSION['user-id'];
$user_fullname = $_SESSION['user-fullname'];
?>

<h2 style="text-align:center;
font-family: 'Lobster', cursive;
font-size: 60px;
color: black;

">Clothes shop</h2>

<br>
<br>
<div class="container">
	<!------   Search bar   ---->
	<div class="search-container">
		<br />
		<div class="form-group row">
			<div class="col-xs-3">
				<input onclick="myFunction()" type="text" name="search_text" id="search_text" placeholder="Search by Shop" class="form-control" / style="width:435%;">
			</div>
			<div class="col-xs-4">


<br>
<br>
<br>




				<span id="btn" hidden><a href="UserArkimormaShop.php"><button onclick="myFunction2()" class="btn btn-primary" 
				 style="margin-left: -290px;">Cancel</button></a></span>
			</div>
			</div>
		</div>
		<br />
		<div id="result" hidden></div>
	</div>

</div>
	
 <script>
function myFunction() {
	document.getElementById("result").hidden = false;
	document.getElementById("btn").hidden = false;
}
function myFunction2() {
	document.getElementById("result").hidden = true;
	document.getElementById("btn").hidden = true;
}



$(document).ready(function(){

 load_data();

 function load_data(query)
 {
	$.ajax({
	 url:"fetchShop.php",
	 method:"POST",
	 data:{query:query},
	 success:function(data)
	 {
		$('#result').html(data);
	 }
	});
 }
 $('#search_text').keyup(function(){
	var search = $(this).val();

	if(search != '')
	{
	 load_data(search);
	}
	else
	{
	 load_data();
	}
 });
});
</script>

<div class="container">
		<h4 style="font-family: sans-serif;">Followed Shop</h4>
		<div class="shops-container container">
	<div class="row">

	 <?php    
	 //followed shops
	 $fol_sql = $dbconnnect->query("SELECT shop_id FROM follow_table WHERE user_id = '$user_id' ORDER BY date_follow DESC");

	 $exist = mysqli_num_rows($fol_sql);
	 if ($exist == 0) {
	 		echo"<i style='margin-left:43%;' ><b>No Followed Shops</b></i>";
	 }
	 while ($row_id = mysqli_fetch_assoc($fol_sql)) {
	 	$follow_id = $row_id['shop_id'];

	 
	
		

	 $sql = "SELECT * FROM sellers WHERE shop_id = ' $follow_id'";
	 $result = mysqli_query($dbconnnect,$sql);

	 while ($row = mysqli_fetch_assoc($result)) {
			$shop_id = $row['shop_id'];
			$image = $row['image'];
			$shop = $row['shop_name'];
			$Province = $row['province']; 
			?>
			 <div class="shops-item col-xs-3">
			 	<div class="shop_content"> 
				<div class="shops-item-content">
					
					<div id="proimg">
							<img src="../assets/SellerAccountImages/<?php echo $image ;?>" >           
					</div>
					<div class="content">   
						<?php $string = $shop;
						if (strlen($string) > 20) {
						$trimstring = substr($string, 0, 20). '...';
						} else {
						$trimstring = $string;
						}
						?>      
						<b><a href="UserSellerShop.php?shop=<?php echo $shop_id ?>"><?php echo $trimstring; ?></a></b>
						<p><?php echo $row['city'] ?> <?php echo $Province ?></p>   

						<br><br>
						<a href="UserChatIndex.php"><button class="btn btn-xs btn-primary">Chat</button></a>
						<a href="UserSellerShop.php?shop=<?php echo $shop_id ?>"><button class="btn btn-xs btn-success">View Shop</button></a>
						<a href="UserReportForm.php?shop=<?php echo $shop_id ?>&cust=<?php echo$user_id ?>&shop_name=<?php echo$row['shop_name'] ?>&customer_name=<?php echo$user_fullname ?>&customer_email=<?php echo$row['email'] ?>"><button class="btn btn-xs btn-warning">Report</button></a>
					</div>
				</div>
			</div>
	
			 </div>

	<?php }

	} ?>
	</div>
</div>

</div>
	<br>
<br>
<hr style="width:80%; ">
<br>
<br>

<h4>Shops</h4>
<div class="shops-container container">
	<div class="row">
	 <?php    
	 //fetching all shops
	 $sql = "SELECT * FROM sellers WHERE status = 'Admit'";
	 $result = mysqli_query($dbconnnect,$sql);

	 while ($row = mysqli_fetch_assoc($result)) {
			$shop_id = $row['shop_id'];
			$image = $row['image'];
			$shop = $row['shop_name'];
			$Province = $row['province']; 
			?>
			 <div class="shops-item col-xs-3">
			 	<div class="shop_content"> 
				<div class="shops-item-content">
					
					<div id="proimg">
							<img src="../assets/SellerAccountImages/<?php echo $image ;?>" >           
					</div>
					<div class="content">   
						<?php $string = $shop;
						if (strlen($string) > 20) {
						$trimstring = substr($string, 0, 20). '...';
						} else {
						$trimstring = $string;
						}
						?>      
						<b><a href="UserSellerShop.php?shop=<?php echo $shop_id ?>"><?php echo $trimstring; ?></a></b>
						<p><?php echo $row['city'] ?> <?php echo $Province ?></p>   

						<br><br>
						<a href="UserChatIndex.php"><button class="btn btn-xs btn-primary">Chat</button></a>
						<a href="UserSellerShop.php?shop=<?php echo $shop_id ?>"><button class="btn btn-xs btn-success">View Shop</button></a>
						<a href="UserReportForm.php?shop=<?php echo $shop_id ?>&cust=<?php echo$user_id ?>&shop_name=<?php echo$row['shop_name'] ?>&customer_name=<?php echo$user_fullname ?>&customer_email=<?php echo$row['email'] ?>"><button class="btn btn-xs btn-warning">Report</button></a>
					</div>
				</div>
			</div>
	
			 </div>

	<?php } ?>
	</div>
</div>

<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<br>
<?php include('../includes/footer.php'); ?>

<script src="../assets/js/jquery.js"></script>
<script src="../assets/plugins/bootstrap/js/bootstrap.js"></script>
<script src="../assets/plugins/fontawesome/js/fontawesome.js"></script>

<script type="text/javascript">
	var btn = $('#button');

$(window).scroll(function() {
  if ($(window).scrollTop() > 300) {
    btn.addClass('show');
  } else {
    btn.removeClass('show');
  }
});

btn.on('click', function(e) {
  e.preventDefault();
  $('html, body').animate({scrollTop:0}, '300');
});


</script>

</body>
</html>