...
if (isset($_POST['submit'])) {
		...
		$upass = $_POST['pword'];
		$upass = md5($upass);
		$sql = "SELECT * FROM sellers where username = '$uname' AND password = '$upass' LIMIT 1";
		$result = mysqli_query($dbconnnect,$sql);

		if($result->num_rows !=0) {
			$row = $result -> fetch_assoc();
			$Seller_verify = $row['verified'];
            $admin_ver = $row['admin_approve'];
			if ($Seller_verify == 1 && $admin_ver == 1) {
				...
				$_SESSION['id'] = $row['shop_id'];
				$_SESSION['shopname'] = $row['shop_name'];
				$_SESSION['sellerlogin'] = $row;
    ...
    $date = date('Y-m-d');
    $Sql = $dbconnnect->query("SELECT * FROM sellers ");
    while ($row = $Sql->fetch_assoc()) {
        if ($row['violation']>=3) {
            ...
            $update = $dbconnnect->query("UPDATE sellers SET status = 'Banned' , Date_Banned = '$date' , Date_end_banned = '$return_date' , violation = '0' WHERE shop_id = '$id'");
          

            $ban_end = $row['Date_end_banned'];
        }


        //$return = strtotime($date_return);
        if ($row['Date_end_banned'] <= $date && $row['status'] == 'Banned') {
            $id=$row['shop_id'];
            $email = $row['email'];
            $username = $row['username'];

           $update = $dbconnnect->query("UPDATE sellers SET status = 'Admit' WHERE shop_id = '$id'");
           if ($update) {
            ...
            mail($to, $subject, $message, $headers);
               ...
               $delete = $dbconnnect->query("DELETE FROM report WHERE shop_id = '$id'");
           }
        }
    }
...
<head>
	<meta charset="utf-8">
	<title>Arkimorma | Login Seller</title>
...
if (isset($_GET['del'])) {
        $username = $_GET['del'];
        $delete = $dbconnnect->query("DELETE FROM sellers WHERE username = '$username'");
